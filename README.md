...
Объединяет два отношения по условию. Например, чтобы получить список студентов и названий их курсов:

$$
Students \bowtie_{Students.StudentID = Courses.StudentID} Courses
$$

Также можно указать символ соединения без условия. В таком случае соединение произойдет по условию равенства всех одноименных атрибутов.
...
- **STUDENT**($student\_id$, $student\_name$, $group\_id$, $year$) (Студент)
- **COURSE**($course\_id$, $title$, $dept$) (Курс (предмет))
- **CLASS**($class\_id$, $group\_id$, $course\_id$, $instructor$, $semester$) (Занятие - запись о том, что конкретный курс у конкретной группы в конкретном семестре ведет конкретный преподаватель)
- **ENROLL**($student\_id$, $course\_id$, $grade$) (Запись на курс - полагаем все курсы опциональными, запись в этой таблице говорит о том, что конкретный студент записался на конкретный курс и получил (получит) по нему оценку)
- **CLUB**($club\_name$, $student\_id$) (Клуб - запись означает, что студент записан в данный клуб)
...
Использовать только $\times$, $\sigma$ и $\pi$.**
...
$$
R = \text{STUDENT} \times \text{ENROLL} \times \text{COURSE}
$$
...
\text{ENROLL}.course\_id = \text{COURSE}.course\_id
}(R)
$$
...
\text{STUDENT} \times \text{ENROLL} \times \text{COURSE}
\bigr)
\Bigr)
$$
...
## Задание 3  
**Найти имена студентов, посещающих все курсы кафедры `"DM"`.**
...
$$
T = \pi_{course\_id}\bigl(\sigma_{dept="DM"}(\text{COURSE})\bigr)
$$

2) Факты записи:

$$
R = \pi_{student\_id,\,course\_id}(\text{ENROLL})
$$
...
\text{STUDENT} \bowtie
\bigl(
\pi_{student\_id,\,course\_id}(\text{ENROLL})
\div
\pi_{course\_id}\bigl(\sigma_{dept="DM"}(\text{COURSE})\bigr)
\bigr)
\Bigr)
$$
...
## Задание 4  
**Найти имена всех студентов второго курса.**
...
## Задание 7  
**Найти имена студентов, которые обучаются в группе `"922"` и находятся на втором курсе.**
...
## Задание 10  
**Найти имена студентов, обучающихся у преподавателя `"Ivanov"`.**

---

## Задание 11  
**Получить список навзаний курсов, которые изучает группа `"922"`.**

---

## Задание 12  
**Найти имена студентов и их оценки по курсу `"Databases"`.**

---

## Задание 13  
**Найти имена студентов, которые состоят хотя бы в одном клубе.**

---

## Задание 14  
**Найти имена студентов, которые состоят в клубах и одновременно обучаются в группе `"026"`.**

---

## Задание 15  
**Найти имена студентов группы `"026"`, которые не состоят ни в одном клубе.**

---

## Задание 16  
**Найти имена студентов, не записанных ни на один курс.**

---

## Задание 17  
**Найти названия курсов, по которым не проводится ни одного занятия.**

---

## Задание 18  
**Найти имена студентов, посещающих все занятия своей группы (т.е. записанных на все курсы, по которым у их группы есть занятие).**

---

## Задание 19  
**Найти имена студентов, имеющих оценку `"10"` хотя бы по одному курсу, и состоящих хотя бы в одном клубе.**

---

## Задание 20  
**Найти имена студентов, состоящих во всех существующих клубах.**
